...
async function runSecurityAnalysis() {
  ...
  if (cliProgramId) {
    ...
    console.log(colors.green(`Analyzing ${contractType} contract with program ID: ${programId}`));
  } else {
...
function runSecurityChecks(contractContent) {
  ...
  const checks = {
    ...
    authorityValidation: {
      ...
      warning: 'Authority validation may be missing! This may allow unauthorized access to funds or operations',
...
function runVulnerabilityScanning(contractContent) {
  ...
  const vulnerabilityPatterns = [
    ...
    },
    {
      ...
      risk: 'MEDIUM',
      recommendation: 'Ensure CPI calls are properly validated and authorized'
    },
    {
  ...
  // Real-world scanning would use specialized tools like VRust and Solana Verifier
  ...
  const anchorIssues = [
    ...
    },
    {
      ...
      description: 'Accounts can be initialized but not closed, leading to resource waste',
      risk: 'MEDIUM',
      recommendation: 'Implement close functionality for all initializable accounts'
    },
    {
...
function generateReport(securityChecks, codeQualityResults, vulnerabilityScanningResults, deploymentStatus, totalScore, maxScore, scorePercent, contractType, programId) {
  ...
  report += `|----------|-------|--------|\n`;
  report += `| Basic Security Checks | ${totalScore}/${maxScore} (${scorePercent}%) | ${scorePercent >= 80 ? '✅ PASS' : scorePercent >= 60 ? '⚠️ WARNING' : '❌ FAIL'} |\n`;
  report += `| Code Quality | ${codeQualityResults.rustfmt.status && codeQualityResults.clippy.status ? '2/2 (100%)' : codeQualityResults.rustfmt.status || codeQualityResults.clippy.status ? '1/2 (50%)' : '0/2 (0%)'} | ${codeQualityResults.rustfmt.status && codeQualityResults.clippy.status ? '✅ PASS' : '⚠️ WARNING'} |\n`;
  report += `| Vulnerability Scanning | ${vulnerabilityScanningResults.passedChecks}/${vulnerabilityScanningResults.totalChecks} (${Math.round(vulnerabilityScanningResults.score * 100)}%) | ${vulnerabilityScanningResults.score >= 0.8 ? '✅ PASS' : vulnerabilityScanningResults.score >= 0.6 ? '⚠️ WARNING' : '❌ FAIL'} |\n`;
  ...
  vulnerabilityScanningResults.findings.forEach(finding => {
    allRecommendations.push(`- 🛡️ **Security Vulnerability**: Fix ${finding.name} (${finding.risk} risk): ${finding.recommendation}`);
  });
